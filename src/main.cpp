...
// Copyright (c) 2013-2017 The Indiacash developers
...
#if defined(NDEBUG)
# error "Indiacash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb8a1e2fe78a6ce54fa2fcdd184c3f1416e31d72e6d130be7589293abd6b239a3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 16); // Indiacash: starting difficulty is 1 / 2^16
...
const string strMessageMagic = "Indiacash Signed Message:\n";
...
{
    // Indiacash: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    if (nHeight < 9)
        return 10000000 * COIN; // Indiacash Company Premine

    int64 nSubsidy = 1* COIN; // Indiacash: 5.4K INC per day thereafter

    // Indiacash: 180M total INC
    if (nHeight > 10000007)
        nSubsidy=0;
...
static const int64 nTargetTimespan = 12 * 60 * 60; // Indiacash: 12 hours
static const int64 nTargetSpacing = 60; // Indiacash: 60 seconds
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespan2Q13 = 60 * 60; // Indiacash: 60 minutes
...
unsigned int static GetNextWorkRequiredIndiacash(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Indiacash: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    ...
    int64_t PastBlocksMin = 60; // Indiacash: look-back 60 minutes
    int64_t PastBlocksMax = 60; // Indiacash: look-back 60 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    } else if (retarget == DIFF_INC) {
        // Retarget using Indiacash style retargeting
        return GetNextWorkRequiredIndiacash(pindexLast, pblock);
    } else  {
        // Retarget using Classic Indiacash style retargeting
        unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
...
void ThreadScriptCheck() {
    RenameThread("indiacash-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Indiacash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb8a1e2fe78a6ce54fa2fcdd184c3f1416e31d72e6d130be7589293abd6b239a3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("1430401846869710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515571309;
        ...
        assert(block.hashMerkleRoot == uint256("0x573f992a0e1cab26691e0c212014ee683706b2aa7cb289cc9edfa4181f0d0034"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x63, 0x42, 0x21, 0x2c }; // Indiacash: switched at 1393657079.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IndiacashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IndiacashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IndiacashMiner(CWallet *pwallet)
{
    printf("IndiacashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("indiacash-miner");
    ...
    try { loop {
        ...
        printf("Running IndiacashMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("IndiacashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IndiacashMiner, pwallet));
}
